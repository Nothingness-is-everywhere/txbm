...
# 步骤3：升级目标指示模板匹配选区 x=68, y=148, w=160, h=72 @ 1080x1920 中心 (148, 184)
# rx=0.0630, ry=0.0771, rw=0.1481, rh=0.0375
_TARGET_INDICATOR_ROI = [0.0630, 0.0771, 0.2111, 0.1146]
    ...
    def _ensure_target_indicator(self) -> bool:
        ...
        while max_iters <= 0 or clicked < max_iters:
            ...
            if matched:
                ...
                logger.info(
                    f"日常升两级：步骤3匹配到升级目标指示 (置信度 {conf:.3f} >= {threshold}){suffix}"
                )
                return True
            h, w = frame.shape[:2]
